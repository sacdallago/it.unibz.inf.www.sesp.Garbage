...
{//Per ogni oggetto Obj
	...
	numberOfGarbageTypes=x[i].getElementsByTagName("Garbage").length;
	...
	{//Per ogni tipo diverso di Garbage
		...
		//Inserisci Tipo  ------ 3.1 
		garbageType[0]=x[i].getElementsByTagName("Garbage")[j].getElementsByTagName("Type")[0].childNodes[0].nodeValue;
		//Inserisci Colore  ------ 3.2
		garbageType[1]=x[i].getElementsByTagName("Garbage")[j].getElementsByTagName("BinColour")[0].childNodes[0].nodeValue;
		//Retrieve dei diversi elementi che andranno successivamente in una var "items"
		numberOfItems=x[i].getElementsByTagName("Garbage")[j].getElementsByTagName("Item").length;
		var items = new Array(numberOfItems);
		for (var k = 0; k<numberOfItems;k++)
		{//Per ogni Item, inseriscilo nella variabile "items"
			items[k]=x[i].getElementsByTagName("Garbage")[j].getElementsByTagName("Item")[k].childNodes[0].nodeValue;
		}
